...
### ۵. اتصال سامانه‌ی تیکت (اختیاری)

اگر می‌خواهید سامانه‌ی تیکت مستقیم از دستیار جواب بگیرد، یک کلید بسازید:

```bash
python3 -c "import secrets; print('mil_' + secrets.token_urlsafe(32))"
```

آن را در `.env` بگذارید و سرویس را ری‌استارت کنید:

```bash
sudo -u aiassist nano /opt/aiassist/.env     # خط INTEGRATION_API_KEYS
sudo systemctl restart aiassist
```

قالب درخواست و پاسخ در [`API.md`](API.md) است. یادتان باشد در تب مستندات،
**محصول** هر فایل را هم درست بگذارید تا جواب هر محصول از راهنمای خودش دربیاید.
